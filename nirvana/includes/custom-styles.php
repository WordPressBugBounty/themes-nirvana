...
function nirvana_body_classes($classes) {
	...
	if ($nirvanas['nirvana_comlabels'] == 0) $classes[] = 'nirvana-comment-placeholders';
...
.entry-content, .entry-summary, .widget-container, .ppbox, .navigation
					{ 
	<?php if ($nirvana_wordspace != 'Default') { ?> word-spacing: <?php echo esc_html($nirvana_wordspace) ?>; <?php } ?>
	<?php if ($nirvana_letterspace != 'Default') { ?> letter-spacing: <?php echo esc_html($nirvana_letterspace) ?>; <?php } ?> }
<?php
...
@media (max-width: 1920px) {
		#branding, #bg_image { display: block; height: auto; max-width: 100%; min-height: inherit !important; }
} <?php }
	return apply_filters( 'nirvana_custom_styles', preg_replace( array( '/(([\w-]+):\s*?;?\s*?([;}]))/i', '/:\s?default/i' ), array( '$3', ':inherit' ), ob_get_clean() ) );
...
<?php } // nirvana_column_frames
	...
	return apply_filters( 'nirvana_custom_pp_styles', preg_replace( array( '/((background-)?color:\s*?)[;}]/i', '/:\s?default/i' ), array( '', ':inherit' ), ob_get_clean() ) );
